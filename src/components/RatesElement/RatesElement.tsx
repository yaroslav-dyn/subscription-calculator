    ...
    return (
      (actualRates &&
        Object.entries(actualRates)
          ...
          .filter(
            (crr) =>
              crr.cur !== displayCurrency &&
              (currencyFilter
                ? crr.cur.toLowerCase().includes(currencyFilter.toLowerCase())
                : true),
          )) ||
      []
    )
    ...
    <div className={`max-w-6xl mx-auto ${classes}`}>
      ...
      <div ref={ratesPanelRef} className={containerClasses}>
        <div className="flex flex-col md:flex-row md:items-center max-md:gap-y-4 md:justify-between mb-6">
          ...
          <div
            className={`flex flex-col md:flex-row gap-2 md:items-center md:justify-between 
            ${!isPage ? 'md:basis-2/3' : 'flex-1'} gap-x-2`}
          >
            {isPage && <CurrencySelectElement classes="md:basis-28" />}
            <input
              className="border border-white rounded-2xl text-white p-2 bg-white/10 backdrop-blur-sm md:basis-24"
              type="number"
              value={coefficiant}
              onInput={(e) =>
                setCoefficiant((e.target as HTMLInputElement).value)
              }
              placeholder="Count ..."
            />
            <input
              ...
              onInput={(e) =>
                setCurrencyFilter((e.target as HTMLInputElement).value)
              }
